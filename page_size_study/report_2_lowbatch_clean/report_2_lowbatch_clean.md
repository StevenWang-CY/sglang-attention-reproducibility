> ⚠️ **Superseded in part by [report_3_dense_sweep_MAIN.md](../report_3_dense_sweep_MAIN/report_3_dense_sweep_MAIN.md).** This report's headline — "page size is near-neutral; `page_size=1` never loses by ≥5%" — holds for the regimes tested here (FlashInfer-centric, CUDA-graph, **low batch**). A denser re-run that adds **high batch** found a clean ≥5% (in fact ~20–25%) `page_size=1` penalty for the **Triton** backend at bs≥16. Read V3 for the current answer; the methodology and contention-artifact lessons below remain valid.

# Does KV-cache `page_size=1` ever lose by ≥5%? — A 4-experiment study

**Report V2 — page size vs decode latency in SGLang, with a Triton backend, a no-CUDA-graph regime, and a measurement-rigor postmortem**
...
| **Model / prompt** | Qwen3-VL-2B-Instruct, ~9.7k-token prompt (`html_request/request_005`), `--disable-radix-cache` |
| **Code** | [measure_batch_latency_offline.py](../../measure_batch_latency_offline.py) (now exposes `--attention-backend triton`), [page_size_study/scripts/make_v2_figures.py](../scripts/make_v2_figures.py) |
| **Companion** | The kernel-level study and its rigor notes are in [report_1_kernel_microbench.md](../report_1_kernel_microbench/report_1_kernel_microbench.md) (V1). |
...
TPOT (decode time per token) is measured by [measure_batch_latency_offline.py](../../measure_batch_latency_offline.py) over `page_size ∈ {1,8,32,128}`, `batch ∈ {1,8,16}`, ~10k context. The Triton sweep needed only a **one-line** harness change (adding `"triton"` to the `--attention-backend` choices; the engine plumbing already supported it).
...
$PY measure_batch_latency_offline.py html_request/long_ctx10k.json \
...
python3 page_size_study/scripts/make_v2_figures.py   # figures from the clean data
```
**Data:** `offline_batch_results/{ps_e2e_5060ti, fi_clean_5060ti, triton_sweep_5060ti, triton_clean_5060ti, ps_nocudagraph_5060ti}/`.
