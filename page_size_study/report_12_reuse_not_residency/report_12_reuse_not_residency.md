...
| **Figures / data** | [fig_reuse_law.png](fig_reuse_law.png) (headline) · [fig_reuse_grid.png](fig_reuse_grid.png) · `offline_batch_results/reuse_profile/` (58 cells) · [SUDO_CHANGES.md](SUDO_CHANGES.md) |
...
- **ps1 vs ps128 sliver** → distinct ps1 = 1.6/0.8% vs ps128 = 0.6/0.1% (both <2%, ≈0); the ps1 excess is the 128× longer index array's own traffic, not KV reuse (report 7: ps128/ps1 DRAM-read ratio 0.998–0.999) (C4).
...
```bash
# phastform, root for ncu. cold = faithful. (idle-gated, audit-logged, password never on disk.)
export PATH=~/venvs/bench_sglang/bin:/usr/local/cuda-12.8/bin:$PATH
echo PW | sudo -S -E bash ~/sglang_log/run_reuse_all.sh   # grid + reuse sweep + cold/warm + ps1
# locally:
python3 page_size_study/scripts/make_reuse_figure.py        # fig_reuse_law.png (headline)
python3 page_size_study/scripts/make_reuse_grid_figure.py   # fig_reuse_grid.png
```
...
**Companion reports:** [report 7](../report_7_sharedprefix_mechanism/report_7_sharedprefix_mechanism.md) (the DRAM↔cache flip this explains), [report 8](../report_8_smallkv_truebatch/report_8_smallkv_truebatch.md) (the cold/warm footprint ladder), [report 11](../report_11_mla_scatter_pagecost/report_11_mla_scatter_pagecost.md) (the MLA cross-head-reuse data).
