# KV-cache `page_size` vs decode latency — dense re-run, and the ≥5% scenario
...
| **Model / prompt** | Qwen3-VL-2B-Instruct, ~9.7k-token prompt (`html_request/long_ctx10k.json`) |
...
| **Supersedes** | The "near-neutral / no ≥5%" conclusion of [report_2_lowbatch_clean.md](../report_2_lowbatch_clean/report_2_lowbatch_clean.md) (correct for the low-batch / FlashInfer regimes it tested; it never reached high batch). |
| **Code / data** | harness [measure_batch_latency_offline.py](../../measure_batch_latency_offline.py) · figures [page_size_study/scripts/make_dense_figures.py](../scripts/make_dense_figures.py) · raw `offline_batch_results/dense_5060ti/` |
...
Raw per-cell JSON + sibling `.log` in `offline_batch_results/dense_5060ti/`: `{fi,tri}_{g,ng}_ps*[ _rev].json` (Block 1), `hb_*` (Block 2), `cf_*` (Block 3). Each JSON is keyed by batch size and records TPOT (median/mean/std/min/max), TTFT, decode_time, total_time, throughput, and per-batch free-memory.
...
python3 page_size_study/scripts/make_dense_figures.py   # regenerates all figures + dense_consistency.md
```
...
Sources: [PagedAttention paper (arXiv 2309.06180)](https://arxiv.org/pdf/2309.06180) · prior study [report_2_lowbatch_clean.md](../report_2_lowbatch_clean/report_2_lowbatch_clean.md) · kernel-level study [report_1_kernel_microbench.md](../report_1_kernel_microbench/report_1_kernel_microbench.md)
