# When is KV-cache `page_size=1` *not* the lowest-latency choice?

**An experiment report on KV-cache page size vs decode latency in SGLang**
...
| **Code** | [benchmark_page_size_attention.py](../../benchmark_page_size_attention.py), [measure_batch_latency_offline.py](../../measure_batch_latency_offline.py), [run_batch_experiments_offline.sh](../../run_batch_experiments_offline.sh) (`page-size-sweep` / new `page-size-sweep-bs`) |
...
The metric is **TPOT** (Time Per Output Token) — pure decode latency, excluding prefill (see [measure_batch_latency_offline.py:304](../../measure_batch_latency_offline.py#L304)).

The motivating observation: running `./run_batch_experiments_offline.sh page-size-sweep` (its default is **batch size 1**), `page_size=1` came out lowest. The question is whether a regime exists where that flips.
...
### 3.1 Isolated kernel microbenchmark — [benchmark_page_size_attention.py](../../benchmark_page_size_attention.py)
...
### 3.2 End-to-end engine — [measure_batch_latency_offline.py](../../measure_batch_latency_offline.py)
Boots `sglang.Engine` **once per page size** and measures real TPOT. Config used on the 5060 Ti:

```
model        Qwen3-VL-2B-Instruct       prompt        9,661 tokens (html_request/request_005)
...
$PY benchmark_page_size_attention.py --output page_size_study/data/page_size_attn_bench_5060ti.json \
...
for PS in 1 8 32 128; do
  $PY measure_batch_latency_offline.py html_request/request_005_20260316_221014/request.json \
    ...
    --output offline_batch_results/ps_e2e_5060ti/results_ps${PS}.json
done

# (3) Figures (local, needs matplotlib)
python3 page_size_study/scripts/make_v1_figures.py
```

A new `page-size-sweep-bs` mode was added to [run_batch_experiments_offline.sh](../../run_batch_experiments_offline.sh) that runs the attention-bound batch sweep and prints, per batch, which page size is lowest and by how much.

A noise/order control was run with `--page-sizes` in forward and reversed order (`page_size_study/data/noise_test/`); analyze with `page_size_study/scripts/make_v1_figures.py` (Fig 5) and the per-cell rank analysis.

**Data artifacts:** `page_size_study/data/page_size_attn_bench_5060ti.json`, `offline_batch_results/ps_e2e_5060ti/results_ps*.json`, `page_size_study/data/confirm_A_5060ti.json` (warm re-measurement), `page_size_study/data/noise_test/{fwd,rev}_*.json` (robustness re-run), `page_size_study/report_1_kernel_microbench/fig*.png`.
...
*(full data in `page_size_study/data/page_size_attn_bench_5060ti.json`; median penalty across all 16 cells = 0.27%, max = 1.08%)*
