#!/usr/bin/env python3
"""Report 7 figure: WHY shared-prefix creates the page_size penalty (and a true batch doesn't).

Reads:
  offline_batch_results/sharedprefix_profile/flashinfer_{shared,distinct}_ps{ps}_bs{B}_kv{L}.csv  (ncu counters)
  offline_batch_results/sharedprefix_profile/lat_{shared,distinct}.json                            (microbench latency)
Writes ../report_7_sharedprefix_mechanism/fig_sharedprefix.png:
...
D = ROOT / "offline_batch_results" / "sharedprefix_profile"
OUT = Path(__file__).resolve().parents[1] / "report_7_sharedprefix_mechanism"
...
axB.set_title("(B) CONSEQUENCE: shared (reuse) ~3× faster; page_size\n"
              "flat in BOTH (FlashInfer) — page penalty surfaces only in\n"
              "the cache-bound regime (Triton, report 3: +11%)",
...
fig.suptitle("Why a shared prefix makes page_size matter (and a true batch doesn't) — Qwen3-VL-2B, RTX 5060 Ti, ncu",
