...
Data sources (all RTX 5060 Ti, Qwen3-VL-2B, ~10k ctx, 5060 Ti):
  FlashInfer graph-ON : offline_batch_results/ps_e2e_5060ti/results_ps*.json   (baseline; bs1/8/16, ps1/8/32/128)
  FlashInfer clean    : offline_batch_results/fi_clean_5060ti/fi_clean_{g,ng}_bs{1,8}_ps{1,128}.json
  Triton graph-ON     : offline_batch_results/triton_sweep_5060ti/tri_10k_g_bs{1,8,16}_ps*.json
  Triton no-graph clean: offline_batch_results/triton_clean_5060ti/tri_clean_ng_bs{8,16}_ps*.json
  FlashInfer no-graph CONTENDED (artifact): offline_batch_results/ps_nocudagraph_5060ti/ctx10k_ps*_nograph.json
"""
import json, glob, re
...
fi = load_named("offline_batch_results/fi_clean_5060ti", r"fi_clean_(g|ng)_bs(\d+)_ps(\d+)")
...
tri_ng = load_named("offline_batch_results/triton_clean_5060ti", r"tri_clean_(ng)_bs(\d+)_ps(\d+)")
...
def fig_contention():
    ...
    cont = {}
    for ps in (1, 128):
        p = ROOT / f"offline_batch_results/ps_nocudagraph_5060ti/ctx10k_ps{ps}_nograph.json"
        if p.exists():
            cont[ps] = _med(p)
