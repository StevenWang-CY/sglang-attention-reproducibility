...
# Run: ssh phastform 'echo PW | sudo -S -E CACHE=all bash ~/sglang_log/profile_mla_pagecost_ncu.sh'
# Never writes the sudo password; logs each privileged action to SUDO_CHANGES.md.
set -u
TARGET_USER=${SUDO_USER:-$(id -un)}
TARGET_HOME=$(eval echo "~$TARGET_USER")
PY=$TARGET_HOME/venvs/bench_sglang/bin/python
NCU=/usr/local/cuda-12.8/bin/ncu
BENCH=$TARGET_HOME/sglang_log/bench_mla_pagecost.py
OUTDIR=$TARGET_HOME/sglang_log/offline_batch_results/pagecost_profile
