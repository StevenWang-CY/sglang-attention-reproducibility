...
# Tags vllm_b{B}_l{L}_blk{blk}.json in offline_batch_results/vllm_smallkv_5060ti/.
set -u
cd ~/sglang_log
source ~/vllm_env.sh
PY=~/venvs/vllm/bin/python
OUT=~/sglang_log/offline_batch_results/vllm_smallkv_5060ti; mkdir -p $OUT
MODEL=~/hf_models/Qwen3-VL-2B-Instruct
...
run(){ local B=$1 L=$2 blk=$3; local mml=$((L+160))
  ...
  timeout 400 $PY measure_vllm_smallkv.py --model-path $MODEL --block-size $blk --batch-size $B \
