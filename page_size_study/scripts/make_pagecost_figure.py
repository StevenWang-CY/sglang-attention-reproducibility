...
Reads:
  offline_batch_results/pagecost_profile/pagecost_gather.json  (kernel: contig vs block128 vs scatter)
  offline_batch_results/pagecost_profile/pagecost_plan.json    (host plan cost vs index length)
  offline_batch_results/pagecost_profile/flashinfer_{contig,scatter}_all_bs{B}_kv{L}.csv  (ncu)
...
DATA = ROOT / "offline_batch_results" / "pagecost_profile"
OUT = Path(__file__).resolve().parents[1] / "report_10_flashinfer_truebatch_pagecost"
