...
Reads:
  offline_batch_results/offwall_profile/flashinfer_distinct_{all,none}_h{QH}k{KH}_ps{ps}_bs{B}_kv{L}.csv
  offline_batch_results/offwall_profile/pagecost_{pat}_all_h{QH}k{KH}_bs{B}_kv{L}.csv
  offline_batch_results/offwall_profile/bob_{model}_all_ps{ps}_bs{B}_kv{L}.csv     (engine ncu)
  offline_batch_results/offwall_profile/lat_offwall_h*_r*.json                     (CUDA-event ladder)
  offline_batch_results/offwall_profile/pagecost_offwall_h*_r*.json                (frag latency)
Writes ../report_13_truebatch_offwall/fig_offwall.png:
  ...
  (C) page_size cost off the wall: ncu kernel-duration ps1/ps128 ratio vs the cell's DRAM%
      (the exposed index-walk latency) + wall-clock ladder ratio (launch-floor hidden).
...
OUT = Path(__file__).resolve().parents[1] / "report_13_truebatch_offwall"
...
axC.set_title("C — page_size off the wall: the per-token index walk costs +1–5% in KERNEL time\n"
              "as DRAM% falls (bytes identical, ratio 1.001 → latency exposure, report-11 mechanism);\n"
