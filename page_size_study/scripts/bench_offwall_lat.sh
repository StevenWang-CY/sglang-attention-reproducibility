...
OUT=$H/offline_batch_results/offwall_profile
mkdir -p $OUT
...
for r in 1 2 3; do
  ...
  sweep $OUT/lat_offwall_h16k8_r${r}.json $H/bench_xqa.py --latency \
  ...
  sweep $OUT/lat_offwall_h16k2_r${r}.json $H/bench_xqa.py --latency \
  ...
  sweep $OUT/lat_offwall_xqa_r${r}.json $H/bench_xqa.py --latency \
  ...
  sweep $OUT/pagecost_offwall_h16k8_r${r}.json $H/bench_pagecost.py --mode gather \
    --num-q-heads 16 --num-kv-heads 8 --patterns contig block128 scatter \
    --cells "1 512" "2 1024" "4 512" "8 4096"
  sweep $OUT/pagecost_offwall_h16k2_r${r}.json $H/bench_pagecost.py --mode gather \
