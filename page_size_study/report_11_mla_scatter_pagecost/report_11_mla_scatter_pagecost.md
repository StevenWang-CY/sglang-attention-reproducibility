...
| **Data** | `offline_batch_results/pagecost_profile/` (`mla_pagecost.json`, `mla_worstcase.json`, `mla_h{H}_{contig,scatter}_all_bs{B}_kv{L}.csv`) |
| **Figure** | [fig_mla_pagecost.png](fig_mla_pagecost.png) · **Audit:** [SUDO_CHANGES.md](SUDO_CHANGES.md) · **Predecessor:** [report 10](../report_10_flashinfer_truebatch_pagecost/report_10_flashinfer_truebatch_pagecost.md) |
...
**Verdict:** the report-10 caveat is **closed** — even MLA, the one regime genuinely off the DRAM wall with distinct KV, keeps `ps1` **under 5 %** (worst case ~4.07 %), and any real paged layout (`block_size ≥ 64`, as SGLang/vLLM MLA use) is free. The study's rule holds: a ≥5 % `ps1` decode penalty requires **cross-sequence reuse (shared prefix)**, not small footprint, small per-token burst, or fragmentation.
...
**The report-10 caveat is closed: across MHA/GQA *and* MLA, FlashInfer + CUDA graph + distinct KV has no ≥5 % `ps1` decode penalty** — MHA ≤1.3 % (bandwidth-saturated, scatter hidden), MLA ≤~4.07 % (latency-bound, scatter partly exposed but bounded), realistic `block≥64` paging free in both. The study's unifying conclusion stands: a ≥5 % `ps1` decode penalty is **reuse-gated (shared prefix)**, not produced by footprint, per-token burst size, batch, context length, or fragmentation.
