...
# flashinfer_tree_sparse attention version — validation path for the new
# page-oriented tree-sparse backend.
...
REPEAT=5
OUTPUT_DIR="offline_batch_results_flashinfer_tree_sparse"
...
# Page-promotion validation config
export TREE_SPARSE_PROMOTE_PAGE_SIZE="${TREE_SPARSE_PROMOTE_PAGE_SIZE:-64}"
export FLASHINFER_ALLOW_JIT_FAILURE=0
RUNTIME_PAGE_SIZE="${RUNTIME_PAGE_SIZE:-64}"
...
echo "========================================================================"
echo "SGLang Offline Batch Latency Experiments (FlashInfer Tree-Sparse Attention)"
echo "========================================================================"
echo "Date: $(date)"
echo "Model: $MODEL_PATH"
echo "Attention backend: flashinfer_tree_sparse"
...
echo "  promote_page_size: $TREE_SPARSE_PROMOTE_PAGE_SIZE"
echo "  runtime_page_size: $RUNTIME_PAGE_SIZE"
echo ""
...
for TOKEN_SIZE in "${TOKEN_SIZES[@]}"; do
    ...
    LOG_FILE="${OUTPUT_DIR}/runtime_offline_flashinfer_tree_sparse_tokens_${TOKEN_SIZE}_${TIMESTAMP}.log"
    ...
    CMD="python measure_batch_latency_offline.py \"$REQUEST_FILE\" \
        ...
        --repeat $REPEAT \
        --output \"$OUTPUT_FILE\" \
        --attention-backend flashinfer_tree_sparse \
        --page-size $RUNTIME_PAGE_SIZE \
...
with open('$TPOT_FILE', 'w') as out:
    ...
    out.write('Attention: flashinfer_tree_sparse\n')
...
echo "  python compare_attention_backends.py offline_batch_results/ offline_batch_results_tree_sparse/ $OUTPUT_DIR/"
echo ""
echo "========================================================================"
