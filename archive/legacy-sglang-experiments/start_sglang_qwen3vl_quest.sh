...
LOG_FILE="${LOG_DIR}/runtime_qwen3vl_quest_${TIMESTAMP}.log"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] =================================================="
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting SGLang with Quest sparse attention"
...
# Start SGLang server with Quest sparse attention
python -m sglang.launch_server \
  ...
  --page-size 64 \
  --enable-hisparse \
  --hisparse-config '{"algorithm": "quest", "backend": "fa3", "top_k": 2048, "page_size": 64, "min_sparse_prompt_len": 2048, "sparsity_ratio": 0.7}' \
