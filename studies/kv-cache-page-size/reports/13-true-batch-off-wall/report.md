...
> **Mechanism clarification (2026-07-17; reports 15–16).** This report contains two distinct paths that must
> not be conflated. The SGLang engine path lowers decode to a page-independent per-token index list and is
> page-flat at the system level. The native FlashInfer ladder calls `plan(P)` and simultaneously changes
> native page-table length, KV layout and planning metadata; its repeatable microsecond differences do not
> uniquely identify “index-walk latency” or prove the mechanism of the SGLang path. Treat that native
> internal attribution as **underdetermined**. The under-parallelism construction, launch-floor
> measurement and engine-level invisibility remain measured results.
...
| **Answer** | (1) **Yes — constructed and verified in-engine.** Qwen2.5-3B (GQA-2) at **B2/L1024**: decode-attn kernel = **40.5–40.8%** DRAM in the microbench and **in-engine (ncu inside `bench_one_batch`, §1 table)**. The governing variable is total decode work **B·L·kv_heads ≲ ~4k head-tokens** (≈2 MB/layer), not footprint per se: the kernel runs out of CTAs (waves/SM ≤ 0.11, occupancy 8%), so DRAM idles — **L2-hit stays ≤ ~8–17% (cold)**, i.e. off-wall ≠ cache-resident. (2) No ≥5% effect appears at wall-clock or engine TPOT in these cells. The native `plan(P)` wrapper's isolated kernel duration does show a repeatable **+5.3%/+5.5%** microsecond spread at the smallest cell and +1–3% through the transition, but it changes page-table length, layout, and planning metadata together. Its unique internal cause is underdetermined, and it is not the SGLang engine lowering. The 105-run graph-ON TPOT sweep has **`ps1` fastest in every cell** (−1…−3.6% vs ps128, both backends; no page ever ≥5% slower). Worst-case `ps1` fragmentation (random token order) is **≤ +3.4%** kernel time, aside from one ±quantum 10 µs corner at +9%; identical `sectors/request` bounds but does not uniquely identify the cause. |
...
| **Data** | `studies/kv-cache-page-size/data/raw/offwall_profile/` (~190 ncu cells incl. the `rep2/`+`rep3/` verification passes + ladders; `nsys/` + `nsys_vast/` timeline traces, §5) · `studies/kv-cache-page-size/data/raw/bench_one_batch_offwall_5060ti/` (TPOT) · [provenance.md](provenance.md) |
...
### 2a. Native `page_size` ladder — repeatable microsecond spread, but its internal cause is not uniquely identified
...
- **Kernel level (ncu duration, cold; deterministic):** `ps1` costs **+1–5%** vs `ps128` through the
  ...
  `dram__bytes_op_read` ps1/ps128 = **1.001–1.005** everywhere, so the measured DRAM-byte total is nearly
  unchanged. But this **native FlashInfer** sweep calls `plan(P)` and changes page-table length, KV tensor
  layout, split/planning metadata and address-generation granularity together. It is not SGLang's
  per-token, page-independent decode lowering. Therefore neither equal DRAM bytes nor the §2b scatter
  control uniquely proves “scattered-index DRAM latency” or a per-token index-walk cause; instruction count,
  planning/dispatch and dependency structure were not held fixed. The repeatable +1–5% duration spread is
  measured, while its unique internal mechanism remains **underdetermined**.
...
- **XQA** (page ≥16 only): this wall-clock ladder sat on the same launch floor and could not resolve a clean
  page trend. It must not be read as proof that XQA is page-insensitive. Report 16 later measured the kernel
  directly and found genuine `global_ld ∝ 1/page` work, exposed in shared/L2-reuse off-wall cells.
...
- The native tc ladder also reports `ps1` **+1.1…+6.9%** kernel time vs `ps128` in several
  off-wall/transition cells (B1/L1024 +6.6%, B2/L2048 +4.1%; DRAM-read ratio 1.004), ~0 at the wall —
  **with a contention caveat**: the tc-grid window partially overlapped engine TPOT runs (the §2b phantom's
  window), so treat these ratios as indicative. Equal DRAM bytes again do not identify a unique
  index-walk-latency mechanism, because `plan(P)` changes multiple native-wrapper variables. Wall-clock: the
...
- Why flat-with-a-tiny-ps1-lead is the measured outcome: per-step DRAM traffic here is
  **weights 6.2 GB vs KV 0.07 GB** (KV ≈ 1% of traffic) — the report-8 roofline; on top of that sits the §2a
  ...
  ~18 ms step (~0.2%). **In the measured SGLang FlashInfer/Triton cells, the page knob remains free at the
  engine level even when the attention kernel is off the DRAM wall.**
...
## 3. What this adds to the study

> **Prior-art note (added 2026-07-15 after an adversarial novelty check).**
> Two of this report's mechanisms are **prior art**, so the items below are *replication + unification on a
> consumer GPU*, not discovery:
> (i) the small-work **under-parallelism / occupancy** mechanism and the missing work-term in the split
> heuristic (§1, §3-pt-1, F5) were published on Hopper in **[arXiv:2604.00028](https://arxiv.org/abs/2604.00028)**
> (Mar 2026), which also ships the work-aware split fix; (ii) the host-side **launch floor** (§2a/§5.1) was
> published across four GPUs in **[arXiv:2605.30571](https://arxiv.org/abs/2605.30571)** (May 2026). The one
> off-wall native-page microsecond spread (whose unique internal cause remains underdetermined) is shown
> **by this report's own TPOT data** to have no end-to-end consequence. Both papers must be cited by any A/B
> submission built on this material.
...
2. **Off-wall is necessary for some small overheads to become visible, but is not sufficient for a
   page-size effect.** The native `plan(P)` ladder has a repeatable microsecond spread whose internal cause is
   unresolved; the SGLang FlashInfer/Triton engine path remains page-flat because its decode work is
   page-independent. Report 16 supplies the clean positive control: XQA's global-load count really scales
   with page count, producing a shared/off-wall kernel spread. Thus the correct rule is conditional on the
   actual backend/lowering, not “only measurable off-wall and only matters under reuse.”
3. **Methodologically:** ncu-inside-`bench_one_batch` (eager, `-k` filter, mid-run launch windows) is now a
   ...
   unmeasurable by CUDA-event ladders — both extend the study's artifact taxonomy (a sixth class: *launch-floor
   masking*).
...
- **Idle-gated, skip-guarded, audit-logged** runners; password never on disk; all root-created files chowned
  back ([provenance.md](provenance.md)).
...
`studies/kv-cache-page-size/data/raw/offwall_profile/nsys/` (phastform, first pass — partially contended) and
...
4. Tooling notes for the record: nsys must match the driver era like ncu (2024.6.2 produced **silently empty**
   ...
   [provenance.md](provenance.md)); and CUPTI *tracing* vs *counters* have different permission surfaces
...
bash studies/kv-cache-page-size/scripts/bench_offwall_lat.sh
# ncu: grids (both arms) + patterns + warm + tensor-core arm (root, ~45 min, 165 cells; resumable):
sudo -E bash studies/kv-cache-page-size/scripts/run_offwall_ncu_all.sh
# independent repro pass for the near-5% ps1 cells (fresh OUTDIR => fresh measurements):
sudo -E env OUTDIR="$PWD/studies/kv-cache-page-size/data/raw/offwall_profile/rep2" CACHE=all TOOL=xqa \
  QH=16 KH=8 CELLS="1 512|2 128|2 1024|1 1024|4 256|8 4096" PAGES="1 128" \
  bash studies/kv-cache-page-size/scripts/profile_offwall_ncu.sh
# engine: ncu inside bench_one_batch (root, ~25 min) — run while nothing else uses the GPU:
sudo -E bash studies/kv-cache-page-size/scripts/profile_bob_ncu.sh
# engine TPOT page sweep (user, ~3 h):
bash studies/kv-cache-page-size/scripts/bob_offwall.sh
# locally:
python3 studies/kv-cache-page-size/scripts/analyze_offwall.py       # tables T1–T7
python3 studies/kv-cache-page-size/scripts/make_offwall_figure.py   # fig_offwall.png
```

Companion reports: [7](../07-shared-prefix-mechanism/report.md) (the wall and
the cache flip) · [8](../08-small-kv-true-batch/report.md) (footprint doesn't lift it)
· [10](../10-flashinfer-page-cost/report.md) /
[11](../11-mla-scatter-page-cost/report.md) (scatter at/off the wall, MLA) ·
[12](../12-reuse-not-residency/report.md) (reuse-not-residency; cold=faithful).
