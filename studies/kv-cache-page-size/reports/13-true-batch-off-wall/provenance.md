...
`studies/kv-cache-page-size/data/raw/offwall_profile/provenance.md` on the box; merged here.
...
- **`run_offwall_ncu_all.sh` as root** (detached, umask 022): ~123 ncu cells into
  `~/sglang_log/studies/kv-cache-page-size/data/raw/offwall_profile/`:
...
- **nsys profiling runs as root** (CUPTI gated by `RmProfilingAdminOnly=1`): two microbench
  traces + two engine traces into `studies/kv-cache-page-size/data/raw/offwall_profile/nsys/`
