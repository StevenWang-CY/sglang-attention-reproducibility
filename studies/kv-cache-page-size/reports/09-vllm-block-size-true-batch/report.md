...
| **Data** | `studies/kv-cache-page-size/data/raw/vllm_smallkv_5060ti/` — `vllm_*` (sweep), `fit_*` (clean re-measure), `vfy_/eag_/clean_` (controls), `probe_blk*` |
...
2. **In a clean true batch, `block_size` is flat — no ≥5% small-page penalty.** Re-measured on KV-dominant cells
   ...
   This matches the measured SGLang true-batch cells (reports 5 & 8: `ps1` tied) and report 8's finding that
   this FlashInfer **decode kernel is page-flat**. The PI's hoped-for clean >5% small-page penalty was not
   observed in these clean vLLM-FlashInfer cells.
...
`block_size` is **flat to ≤0.2 %** — including 5.6 GB-KV cells where KV ≫ the 4.1 GB weights. Thus the small
page carries **no ≥5% penalty in these clean vLLM true-batch cells**.
...
- **Consistent with reports 5, 7, 8 (and confirmed cross-engine):** the measured clean SGLang and
  vLLM-FlashInfer true-batch cells have **no ≥5% small-page decode penalty**. Reports 15–16 later retract the
  old Triton whole-call interpretation and identify the positive case more precisely: a page-aware kernel
  (XQA) in a shared/off-wall regime.
...
## 6. Reproduce
```bash
# phastform — sm120 vLLM+FlashInfer env (venv-local, no sudo):
source studies/kv-cache-page-size/scripts/vllm_env.sh
bash studies/kv-cache-page-size/scripts/vllm_smallkv_sweep.sh   # initial bs×block sweep (apparent break)
bash studies/kv-cache-page-size/scripts/vllm_verify.sh          # reversed-order + graph ON/OFF controls
# locally:
python3 studies/kv-cache-page-size/scripts/analyze_vllm_smallkv.py     # tables (+ check "Maximum concurrency >= B" per log)
python3 studies/kv-cache-page-size/scripts/make_vllm_smallkv_figure.py # fig_vllm_smallkv.png (headroom classification + clean cells)
```

The clean-fit pass was historically driven by `vllm_fit.sh`; that wrapper was not retained. Its local-only
outputs remain catalogued in the data manifest, so this report does not present the clean-fit pass as a
fully reproducible command from a fresh clone.

Companion reports: [report_5](../05-true-batch/report.md),
[report_7](../07-shared-prefix-mechanism/report.md),
[report_8](../08-small-kv-true-batch/report.md) (SGLang true-batch & mechanism — same verdict: no clean true-batch page penalty).
