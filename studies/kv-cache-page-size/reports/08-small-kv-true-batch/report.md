# Report 8 — Small-KV true-batch stress test: does `page_size=1` lose its tie?

> **Scope correction (2026-07-17; report 16).** The measured FlashInfer/Triton true-batch cells and their
> memory-regime evidence remain valid. Any wording below that turns them into the universal rule
> “page cost is exclusive to shared-prefix + Triton” is superseded. Page behavior depends on whether the
> actual kernel consumes engine pages. Report 16 shows XQA as a genuinely page-aware path; the conclusion
> here is limited to the tested build, backends, layouts and shapes.
...
| **Answer** | **No in the tested construction.** Shrinking the footprint alone did not reliably leave the DRAM regime: the engine remained weight-bound and the cold isolated KV stream remained bandwidth-bound. In the warm cache-resident control, `ps1` still tied `ps128`. No ≥5% `ps1` penalty appeared in this matrix. |
...
| **Data** | `studies/kv-cache-page-size/data/raw/bench_one_batch_smallkv_5060ti/bobk_*.jsonl` · `studies/kv-cache-page-size/data/raw/smallkv_profile/` (`lat_distinct_*.json`, ncu `*.csv`) |
| **Figures** | [fig_smallkv_kernel.png](fig_smallkv_kernel.png) (headline: kernel + ncu) · [fig_smallkv_engine.png](fig_smallkv_engine.png) (engine TPOT + roofline) · [provenance.md](provenance.md) |
...
kernel is page-agnostic in the measured regimes. This bounds the tested FlashInfer/Triton construction;
page-aware kernels such as report 16's XQA require a separate measurement.
...
(`ps1` marginally *faster*) at −0.8 … −4.4 % (FlashInfer) and −1.1 … −4.2 % (Triton), an ordering at the
noise/engine-metadata floor rather than a resolved page effect. The roofline panel of
[fig_smallkv_engine.png](fig_smallkv_engine.png) shows why the measured cells are flat: the KV is only
**1–48 %** of per-step DRAM traffic (B8/L256 → KV 0.24 GB vs weights
...
Every cell ≤ +0.8 % except the **B1/L256** 1 MB corner (+2.3 %), where the ~0.13 ms kernel is dominated by
fixed launch/planning overhead; the small deviation shrinks with cell size. This run does not isolate a
page-dependent host term, and report 10 shows that the SGLang index length is page-independent. **No
footprint in this matrix produces a ≥5% `ps1` penalty, including the deeply L2-resident controls.**
...
- **Current reconciliation (reports 15–16):** leaving the DRAM wall is not sufficient. A backend must also
  perform page-dependent work. The measured SGLang FlashInfer/Triton lowering is page-invariant; XQA is the
  positive page-aware example. This report's small-footprint construction did not create page-dependent
  work and remained flat.
- **vAttention §3.3.2** reports the *opposite sign* for independent sequences — vLLM's paged decode kernel is
  *faster* with **smaller** blocks (better L1 hit). We see the faint echo of that sign, never a penalty: `ps1` is
  marginally *faster* in several distinct cells (B8/L1024 −0.5 %, B32/L1024 −0.1 %, B32/L2048 −0.4 %). The
  effect is tiny here because this SGLang/FlashInfer lowering uses a page-independent per-token index list;
  allocator granularity barely changes the inner gather, so neither a small-page penalty nor a large L1 win
  appears.

**Unifying statement:** the measured result is backend- and regime-dependent. Small distinct-KV
FlashInfer/Triton cells stay page-flat; shared reuse changes the memory regime but causes a page effect only
when the selected kernel performs page-dependent work.
...
- **Triton kernel, off-ceiling-by-small-footprint, is covered by the engine only.** The Triton decode kernel
  ...
  *engine* small-KV cells (Tier A) are weight-bound and show `ps1` tied/fastest. Reports 15–16 later establish
  that shared/cache-bound operation alone does not create a Triton page penalty; page-dependent kernel work
  must be demonstrated separately. We do not claim a clean isolated-Triton small-distinct measurement.
...
bash studies/kv-cache-page-size/scripts/bob_smallkv.sh         # -> studies/kv-cache-page-size/data/raw/bench_one_batch_smallkv_5060ti/bobk_*.jsonl
...
python studies/kv-cache-page-size/scripts/bench_xqa.py --latency --kv-mode distinct --backends flashinfer --page-sizes 1 2 4 8 16 32 64 128 \
...
sudo -E CACHE=all  bash studies/kv-cache-page-size/scripts/profile_smallkv_ncu.sh
sudo -E CACHE=none bash studies/kv-cache-page-size/scripts/profile_smallkv_ncu.sh
# locally:
python3 studies/kv-cache-page-size/scripts/analyze_bob_smallkv.py            # engine tables + roofline + paired CIs
python3 studies/kv-cache-page-size/scripts/make_bob_smallkv_figure.py        # fig_smallkv_engine.png
python3 studies/kv-cache-page-size/scripts/make_smallkv_kernel_figure.py     # fig_smallkv_kernel.png (headline)
```

Companion reports: [report_5](../05-true-batch/report.md) (true batch: `ps1` tied,
no ≥5 %), [report_7](../07-shared-prefix-mechanism/report.md) (the DRAM↔cache flip
— shared prefix is what moves decode on-chip). This report closes the tested footprint route: shrinking KV
alone did not move these true-batch cells off the DRAM wall, and `ps1` remained flat.
