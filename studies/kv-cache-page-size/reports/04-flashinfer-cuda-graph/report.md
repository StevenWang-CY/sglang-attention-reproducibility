# Report 4 — Historical FlashInfer whole-call observation at long context

> **Supersession notice (2026-07-17; report 15).** The paired +8.2/+8.4/+9.2% values below are real
> **whole-call metric** observations, but they are not steady decode penalties. Per-step lines in those same
> three gray logs give ps1-vs-ps128 **−0.7/−0.2/−0.7%**. The old TPOT interval folded the cold shared-prefix
> admission ramp into “decode.” Consequently the `ctx/page_size` page-table-walk/coalescing mechanism in
> this report is **retracted**; SGLang's studied FlashInfer lowering used a page-independent per-token index
> list and planned decode with page size 1. Only ps1/ps128 has same-run steady closure here; the steady
> ordering of ps8/ps32 is not established by the old summary. See
> [report 15](../15-log-forensics-and-upper-bound/report.md).
>
> The remainder is retained as the original report text. Its unqualified decode and mechanism conclusions
> are superseded by this notice.
...
| **Code / data / figure** | [measure_batch_latency_offline.py](../../scripts/measure_batch_latency_offline.py) · [studies/kv-cache-page-size/scripts/make_flashinfer_fullsweep_figure.py](../../scripts/make_flashinfer_fullsweep_figure.py) · `studies/kv-cache-page-size/data/raw/gen_models_5060ti/{fgf_39k_ps*, fgr_r*_ps*}` |
...
The original 39k-token prompt file (`html_request/long_ctx58k.json`) was machine-local and is not part of
the published artifact set. The command is retained as an exact historical recipe, but requires supplying
an equivalent prompt file.
...
python3 studies/kv-cache-page-size/scripts/make_flashinfer_fullsweep_figure.py   # mean ± std per page + the figure
```
...
Companion: the full dense study [03-dense-sweep.md](../03-dense-sweep/report.md) (Triton ≥5%, the FlashInfer eager corner, the cross-model generalization, the vLLM block_size result).
