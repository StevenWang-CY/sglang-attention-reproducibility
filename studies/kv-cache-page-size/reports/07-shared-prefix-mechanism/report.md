# Report 7 — Shared-prefix versus distinct-KV memory regimes

> **Scope correction (2026-07-17; reports 15–16).** The measured shared-vs-distinct DRAM/L2 counters in
> this report remain valid and important: shared physical KV creates intra-kernel L2 reuse, whereas
> distinct KV is near the DRAM wall. The former conclusion that this fact explains reports 3/4's large
> page-size penalties is retracted: those numbers were cold-admission/TPOT artifacts and their steady
> FlashInfer/Triton decode was page-flat. Being off-wall is **not sufficient** for page sensitivity. A
> backend must also perform page-dependent work. Report 16's XQA load count is the positive control; the
> studied FlashInfer/Triton paths are the page-invariant controls.

> PI ask (relayed by Chuyue): use profiling to explain the difference between shared-prefix and true-batch
> memory regimes. The retained answer is the counter result: sharing physical KV creates intra-kernel reuse
> and moves traffic away from DRAM. Later reports show that this regime shift does not by itself imply a
> page-size latency effect.

| | |
|---|---|
| **Question** | How do shared-prefix and distinct-KV decode differ in memory behavior? |
...
| **Headline** | **shared → high intra-kernel L2 reuse** (L2-hit ~86–94 %, DRAM ≪ peak); **distinct → DRAM-bandwidth-bound** (~95 % of peak DRAM, L2-hit ~0 %). This establishes two memory regimes, not a universal page-effect rule. |
| **Figure / data** | [fig_sharedprefix.png](fig_sharedprefix.png) · `studies/kv-cache-page-size/data/raw/sharedprefix_profile/` (ncu `.csv`, `lat_*.json`) · [provenance.md](provenance.md) |
...
The toggle proves why the **memory regimes** differ: a true batch has no cross-sequence KV reuse. It does
not prove a page-size penalty; reports 15–16 establish that page sensitivity additionally requires a
backend that performs page-dependent work.
...
cache-bandwidth/compute regime, ~3× faster (L1/L2 BW ≫ HBM BW). This off-wall regime can expose additional
work if a backend actually performs page-dependent operations; FlashInfer/Triton did not in the studied
lowering, while report 16's XQA path did. (Data:
`sharedprefix_profile/sol/`.)

## The consequence for latency
...
- **In the measured DRAM-bound (distinct) regime the page pattern is ~invisible.** For the FlashInfer kernel
  measured here, `ps1` vs `ps128` is flat to <0.5 % in every distinct cell; report 5 likewise finds
  FlashInfer/Triton true-batch cells flat within the measurement scope. Earlier report-3 attribution of a
  residual Triton page gather cost is superseded by report 15's same-log steady-decode closure.
- **In the off-wall shared regime, page-dependent work would be visible if it exists.** The measured
  FlashInfer/Triton lowering performs page-invariant steady work, so reports 3/4 remain page-flat despite
  this regime. Report 16's XQA kernel supplies the positive control: its load count changes with page size
  and its shared/off-wall latency changes accordingly.

The unifying rule is therefore two-factor: the memory regime determines whether extra work can be exposed,
and the backend/lowering determines whether page-dependent work exists at all.
...
- The 0.998–0.999 DRAM-byte ratio is effectively unity and does not establish a page-size-dependent index
  term. In this SGLang lowering, both page sizes use the same per-token B×L index list (report 10).
- Clean single-kernel latency is **tied to ±0.8 %** with no systematic exact-vs-partial-page pattern
  (inconsistent sign across L).

Thus `ps1` is tied with large pages in the measured true-batch cells. The ≤1% engine ordering (`bobs`) is
at the noise/metadata floor, not a demonstrated index/gather disadvantage or a real `ps1` kernel advantage.
(Data: `sharedprefix_profile/pad2/`.)
...
- **ncu**: `--profile-from-start off` + `--single` (one launch wrapped in `cudaProfilerStart/Stop`); metrics
  ...
  `sm__throughput…`, `gpu__time_duration.sum`. Run as root (RmProfilingAdminOnly=1); see provenance.md.
...
- The microbench reproduces the **first-order** mechanism (the DRAM↔cache flip) and the **3× shared speedup**
  cleanly, but FlashInfer's *kernel* shows no `ps1` page penalty even in shared mode — consistent with the
  later same-run closure showing page-flat steady decode. The old report-3/4 causal interpretation is not
  supported; this report establishes the memory-regime contrast only.
- Engine cross-check (gray, radix ON vs `--disable-radix-cache`, eager) confirmed the radix control works
  ...
  3 k cells were low-signal/noisy, so the **ncu microbench is the primary evidence** for the regime contrast.
...
# latency (no profiler):
python studies/kv-cache-page-size/scripts/bench_xqa.py --latency --backends flashinfer --kv-mode shared   --page-sizes 1 128 --batch-sizes 8 --seq-lens 6144 39168
python studies/kv-cache-page-size/scripts/bench_xqa.py --latency --backends flashinfer --kv-mode distinct --page-sizes 1 128 --batch-sizes 8 --seq-lens 6144 39168
# ncu counters (root):
sudo -E bash studies/kv-cache-page-size/scripts/profile_sharedprefix_ncu.sh
# locally:
python3 studies/kv-cache-page-size/scripts/make_sharedprefix_figure.py     # fig_sharedprefix.png
```

Companion reports: [report 3](../03-dense-sweep/report.md) and
[report 4](../04-flashinfer-cuda-graph/report.md) retain the historical whole-call artifacts
(Triton +11–25%, FlashInfer +8.6%); [report 5](../05-true-batch/report.md) contains the measured
FlashInfer/Triton true-batch scope.
