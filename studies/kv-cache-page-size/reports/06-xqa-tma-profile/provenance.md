...
appends a timestamped line to `~/sglang_log/provenance.log` on the box for every privileged
...
1. Profiler outputs (`*.ncu-rep`, `*.csv`, `*.stdout`) under
   `~/sglang_log/studies/kv-cache-page-size/data/raw/xqa_profile/` — created root-owned, then `chown`ed back
...
- **Privileged action:** `sudo bash profile_xqa_ncu.sh` (detached, umask 022 → outputs world-readable).
  ...
  + full-set cells → `studies/kv-cache-page-size/data/raw/xqa_profile/*.{csv,ncu-rep}`. No persistent system change beyond
  output files (chowned to wangcy07 at end). Every ncu invocation logged in `~/sglang_log/provenance.log`.
...
- Once connectivity returns: run `sudo -E bash ~/sglang_log/profile_xqa_ncu.sh`; this file and
  the on-box `provenance.log` will record the exact `ncu` invocations and their footprint.
...
- files written (root->chowned to wangcy07): studies/kv-cache-page-size/data/raw/xqa_profile/*.csv, *.ncu-rep
