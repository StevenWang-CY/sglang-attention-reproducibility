...
| **Data** | `studies/kv-cache-page-size/data/raw/xqa_profile/` — `*.csv` (108 XQA/FI counter cells), `survey*.csv` (stack survey) |
| **Figures** | [fig_xqa_tma.png](fig_xqa_tma.png) (XQA vs FI counters) · [fig_xqa_latency.png](fig_xqa_latency.png) (latency) |
| **Audit** | [provenance.md](provenance.md) — every privileged action + the one persistent change (matched ncu install) |
...
Reports 3–5 originally hypothesized that small pages would hurt when decode is access-pattern-bound.
Reports 14–15 later showed that the report-3/4 headline values were whole-call admission artifacts, not
steady-decode penalties. What report 6 establishes is narrower: in the measured distinct-KV XQA/FI cells,
both kernels are ~95% HBM-bound and their latency is page-flat. Report 16 later shows XQA becoming
page-sensitive in measured shared/off-wall cells because it performs page-dependent load work.
...
- Latency differences across pages are ≤1.5% in this matrix. Practical takeaway:
  *page_size is ~free for the measured true-batch XQA/FI cells on this hardware.*
...
- **ncu blocker root-caused:** ncu 2025.1.1 failed `LibraryNotLoaded` on the CUDA-13 driver → fixed by
  installing `nsight-compute-2025.3.1` (`cuda-nsight-compute-13-0`). See provenance.md.

## Reproduce
```bash
# sudo-free (latency / correctness / page constraint):
python studies/kv-cache-page-size/scripts/bench_xqa.py --validate
python studies/kv-cache-page-size/scripts/bench_xqa.py --latency --backends xqa flashinfer \
    --page-sizes 1 16 32 64 128 --batch-sizes 1 8 32 64 --seq-lens 1024 4096 16384 \
    --output studies/kv-cache-page-size/data/raw/xqa_profile_5060ti/xqa_latency.json

# counters — needs a CUDA-13-matched ncu as root (logs every privileged action):
sudo -E bash studies/kv-cache-page-size/scripts/profile_xqa_ncu.sh       # XQA vs FlashInfer sweep
sudo -E bash studies/kv-cache-page-size/scripts/profile_tma_survey_ncu.sh # stack-wide TMA survey
python studies/kv-cache-page-size/scripts/tma_positive_control.py        # Triton TMA control
# static corroboration (no GPU/root): cuobjdump --dump-sass <kernel>.so | grep -c UTMALDG

# locally:
python3 studies/kv-cache-page-size/scripts/analyze_xqa_ncu.py
python3 studies/kv-cache-page-size/scripts/make_xqa_figure.py
```
