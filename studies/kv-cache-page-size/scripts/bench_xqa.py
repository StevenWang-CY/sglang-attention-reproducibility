...
Reports 6 and 16 of the page-size study: profile the XQA decode kernel and
compare it to the page-invariant FlashInfer fused decode kernel, in a TRUE batch
...
Backends
  xqa        flashinfer.decode.trtllm_batch_decode_with_kv_cache(backend="xqa")
             -> the XQA kernel (profiler name: kernel_mha); report 6 found no TMA on sm120.
...
# ---------------------------------------------------------------------------
def _perm(n, device):
    """Deterministic page permutation (scatter analog). Fixed seed for reproducibility."""
...
def build_flashinfer(B, L, page_size, device, K=None, V=None, kv_mode="distinct",
                     tensor_cores=False, page_order="contig"):
    ...
    if kv_mode == "shared":
        base_idx = torch.arange(ppr, dtype=torch.int32, device=device)  # pages [0,ppr)
        if page_order == "shuffle" and K is None:                        # scatter the access order
            base_idx = _perm(ppr, device).to(torch.int32)
        kv_indices = base_idx.repeat(B)                                  # all seqs -> same (shuffled) pages
    else:
        if page_order == "shuffle" and K is None:                        # scatter physical pages per position
            kv_indices = _perm(n_phys, device).to(torch.int32)
        else:
...
def build_xqa(B, L, page_size, device, backend="xqa", K=None, V=None, kv_mode="distinct",
    ...
    if kv_mode == "shared":
        base_idx = torch.arange(ppr, dtype=torch.int32, device=device)
        if page_order == "shuffle" and K is None:
            base_idx = _perm(ppr, device).to(torch.int32)
        block_tables = base_idx.repeat(B).view(B, ppr)
    else:
        if page_order == "shuffle" and K is None:
            block_tables = _perm(n_phys, device).to(torch.int32).view(B, ppr)
        else:
        ...
        for B in args.batch_sizes:
            ...
            for be in args.backends:
                for ps in args.page_sizes:
                    key = f"{be}_{args.kv_mode}_{args.page_order}_ps{ps}_bs{B}_kv{L}"
                    try:
                        run = build_flashinfer(B, L, ps, dev, kv_mode=args.kv_mode,
                                               tensor_cores=args.tensor_cores, page_order=args.page_order) \
                            if be == "flashinfer" \
                            else build_xqa(B, L, ps, dev, backend=args.backend, kv_mode=args.kv_mode,
                                           page_order=args.page_order)
                        lat = time_run(run, q)
                        results[key] = dict(backend=be, kv_mode=args.kv_mode, page_order=args.page_order,
                                            page_size=ps, batch_size=B,
                                            seq_len=L, latency_ms=lat,
                                            num_q_heads=NUM_Q_HEADS, num_kv_heads=NUM_KV_HEADS,
                                            tensor_cores=args.tensor_cores)
                        print(f"{be:10s} {args.kv_mode:8s} {args.page_order:7s} ps={ps:4d} bs={B:3d} kv={L:6d} -> {lat:.4f} ms")
                        del run
                        ...
                        results[key] = dict(backend=be, kv_mode=args.kv_mode, page_order=args.page_order,
                                            page_size=ps, batch_size=B,
                                            seq_len=L, error=msg)
                        print(f"{be:10s} {args.kv_mode:8s} {args.page_order:7s} ps={ps:4d} bs={B:3d} kv={L:6d} -> ERR {msg}")
                        torch.cuda.empty_cache()
            del q
            torch.cuda.empty_cache()
...
def main():
    ...
    p.add_argument("--page-order", default="contig", choices=["contig", "shuffle"],
                   help="contig = pages in natural order; shuffle = randperm page mapping "
                        "(scatter analog for a page>=16 kernel; latency/single paths only)")
    # latency sweep
