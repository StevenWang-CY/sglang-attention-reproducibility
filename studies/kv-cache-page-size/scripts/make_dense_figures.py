...
Reads studies/kv-cache-page-size/data/raw/dense_5060ti/{config}_ps{ps}[_rev].json  (Block 1, radix OFF)
...
def load_old():
    ...
    for f in glob.glob(str(ROOT / "studies/kv-cache-page-size/data/raw/fi_clean_5060ti/fi_clean_ng_*.json")):
    ...
    for f in glob.glob(str(ROOT / "studies/kv-cache-page-size/data/raw/triton_clean_5060ti/*.json")):
...
def consistency(data, old):
    ...
    txt = "\n".join(lines)
    (Path(__file__).resolve().parents[1] / "data" / "curated" / "dense_consistency.md").write_text(txt)
    print(txt)
