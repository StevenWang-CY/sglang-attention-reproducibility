...
Inputs (relative to repo root):
  ...
  studies/kv-cache-page-size/data/raw/ps_e2e_5060ti/results_ps{1,8,32,128}.json - end-to-end SGLang engine TPOT, 5060 Ti

Outputs: 01-kernel-microbenchmark/fig*.png
"""
import json, glob, re
...
OUT = Path(__file__).resolve().parents[1] / "reports" / "01-kernel-microbenchmark"  # ../01-kernel-microbenchmark
DATA = Path(__file__).resolve().parents[1] / "data" / "curated"  # ../data (small local data)
...
dc = load_kernel(DATA / "datacenter-kernel-benchmark" / "results.json")
