...
Reads studies/kv-cache-page-size/data/raw/bench_one_batch_5060ti/bob_*.jsonl
...
Writes into ../05-true-batch/ :
  fig_bob_truebatch.png  — (A) Triton TPOT vs page, (B) FlashInfer TPOT vs page,
                            (C) historical shared-prefix whole-call values beside
                                true-batch steady decode, explicitly marked non-equivalent.
"""
import json, glob, re
...
DATA = ROOT / "studies" / "kv-cache-page-size" / "data" / "raw" / "bench_one_batch_5060ti"
OUT = Path(__file__).resolve().parents[1] / "reports" / "05-true-batch"
...
# ---- Panel C: historical whole-call metric vs true-batch steady decode ----------
# The shared-prefix values below are the superseded whole-call metrics from reports 3/4.
# Report 15 shows their same-run steady decode was page-flat; they are retained only to
# make the measurement-level discrepancy visible, not as a causal regime comparison.
...
b1 = axB.bar(x - w/2, sp_vals, w, label="historical shared-prefix whole-call metric (superseded)",
             color="indianred")
b2 = axB.bar(x + w/2, tb_vals, w, label="true-batch steady decode (bench_one_batch)", color="steelblue")
...
axB.set_ylabel("ps1 vs ps128 (%) — different measurement levels")
axB.set_title("Historical shared-prefix bars include the admission-ramp artifact (reports 14–15)",
              fontweight="bold")
