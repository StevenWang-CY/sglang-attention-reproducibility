...
Inputs:
  studies/kv-cache-page-size/data/raw/ps_e2e_5060ti/results_ps*.json        - baseline (FlashInfer, CUDA graph ON)
  studies/kv-cache-page-size/data/raw/ps_nocudagraph_5060ti/ctx*_ps*_*.json - Exp2 (FlashInfer, graph ON/OFF, long ctx, bs1)
  studies/kv-cache-page-size/data/raw/triton_sweep_5060ti/tri_*.json        - Exp3 (Triton)
  studies/kv-cache-page-size/data/raw/vllm_blocksize_5060ti/*.json          - Exp4 (vLLM)
Outputs: 02-low-batch-baseline/fig6_from_zero.png, fig7_triton_vs_fi.png, fig8_graph_ctx.png,
...
def load_nocg():
    ...
    out = {}
    for f in glob.glob(str(ROOT / "studies/kv-cache-page-size/data/raw/ps_nocudagraph_5060ti/ctx*_ps*_*.json")):
...
def load_triton():
    ...
    out = {}
    for f in glob.glob(str(ROOT / "studies/kv-cache-page-size/data/raw/triton_sweep_5060ti/tri_*.json")):
...
def load_vllm():
    ...
    out = {}
    for f in glob.glob(str(ROOT / "studies/kv-cache-page-size/data/raw/vllm_blocksize_5060ti/*.json")):
        try:
            j = json.load(open(f))
        except Exception:
            continue
...
def fig6():
    base = load_e2e("studies/kv-cache-page-size/data/raw/ps_e2e_5060ti")
    if not base:
...
def fig8():
    nocg = load_nocg(); base = load_e2e("studies/kv-cache-page-size/data/raw/ps_e2e_5060ti")
    tri = load_triton()
    if not nocg:
...
def fig7():
    tri = load_triton(); base = load_e2e("studies/kv-cache-page-size/data/raw/ps_e2e_5060ti")
    if not tri:
