...
NSYS="${NSYS_BIN:-$(ls -d /opt/nvidia/nsight-systems/*/bin/nsys | sort -V | tail -1)}"
PY="${PYTHON_BIN:-python}"
BENCH="${BENCH_SCRIPT:-$SCRIPT_DIR/bench_xqa.py}"
OUT="${OUTPUT_DIR:-/workspace/nsys_out}"
M="${MODEL_PATH:-/workspace/Qwen2.5-3B-Instruct}"
mkdir -p "$OUT"
cd "$OUT"

"$NSYS" profile -t cuda --force-overwrite true -o mb_cudacore "$PY" "$BENCH" --latency \
...
"$NSYS" profile -t cuda --force-overwrite true -o mb_tc "$PY" "$BENCH" --latency \
...
"$NSYS" profile -t cuda --force-overwrite true -o eng_b2l1024 "$PY" -m sglang.bench_one_batch \
...
"$NSYS" profile -t cuda --force-overwrite true -o eng_b8l4096 "$PY" -m sglang.bench_one_batch \
