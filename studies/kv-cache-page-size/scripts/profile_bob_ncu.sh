...
#
# WHY ROOT: RmProfilingAdminOnly=1. User-scoped cache variables keep generated files recoverable.
# Run from the repository root: sudo -E bash studies/kv-cache-page-size/scripts/profile_bob_ncu.sh
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STUDY_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
TARGET_USER=${SUDO_USER:-$(id -un)}
TARGET_HOME=$(eval echo "~$TARGET_USER")
PY="${PYTHON_BIN:-$TARGET_HOME/venvs/bench_sglang/bin/python}"
NCU=${NCU:-$(ls -d /opt/nvidia/nsight-compute/*/ncu 2>/dev/null | sort -V | tail -1)}
OUTDIR="${OUTPUT_DIR:-$STUDY_ROOT/data/raw/offwall_profile}"
...
export HF_HOME="${HF_HOME:-$TARGET_HOME/.cache/huggingface}"
export TORCH_HOME="${TORCH_HOME:-$TARGET_HOME/.cache/torch}"
