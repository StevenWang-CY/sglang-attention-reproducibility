#!/usr/bin/env python3
"""Historical cross-model whole-call figures (CUDA graph ON, ~10k ctx, shared prefix).

These reproduce report 3's original harness metric. Reports 14–15 later show that this metric folded
the admission ramp into TPOT and is not evidence of a steady decode-kernel page penalty.
Data: studies/kv-cache-page-size/data/raw/gen_models_5060ti/B_* and dense_5060ti/hb_* (Qwen3-VL-2B reference)."""
import json, os, glob
...
GEN = ROOT / "studies/kv-cache-page-size/data/raw/gen_models_5060ti"
DENSE = ROOT / "studies/kv-cache-page-size/data/raw/dense_5060ti"
VLLM = ROOT / "studies/kv-cache-page-size/data/raw/vllm_blocksize_5060ti"
OUT = Path(__file__).resolve().parents[1] / "reports" / "03-dense-sweep"  # ../03-dense-sweep
...
def fig_generalization():
    ...
    ax.set_ylabel("historical whole-call ps1 vs ps128 (%)")
    ax.set_title("Historical harness metric across models (superseded as steady-decode evidence)\n"
                 "solid = Triton; dashed = FlashInfer; see reports 14–15 for same-log closure.",
                 fontweight="bold")
...
def fig_driver():
    # Descriptive association in the historical whole-call metric; not a decode mechanism.
    pts = []
    for name, d, t1, t128, f1, f128, kv, L, col in MODELS:
        p = pen(d, t1, t128, 32)
        if p is not None:
    ...
    ax.set_xlabel("number of layers"); ax.set_ylabel("historical whole-call difference @ bs32 (%)")
    ax.set_title(f"Historical association with layer count (r = {r:+.2f})", fontweight="bold")
    ax.grid(alpha=0.3)
    # right: vs KV/token
    ax = axes[1]
    for name, kv, L, p, col in pts:
    ...
    ax.set_xlabel("KV-cache bytes per token (KB)"); ax.set_ylabel("historical whole-call difference @ bs32 (%)")
    ax.set_title(f"Does NOT track KV/token  (Pearson r = {r2:+.2f})", fontweight="bold")
    ax.grid(alpha=0.3)
    fig.suptitle(
        "Historical whole-call association (not a steady-decode mechanism): layers vs KV bytes",
        fontweight="bold",
    )
